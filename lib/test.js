...
//                var geoCache = getGeoCache(lat, lng)
                tweet.geonames = retrieveGeoNames(tweet, coordinates[0], coordinates[1]);
            }
        });
    };

    var retrieveGeoNames = function(tweet, lat, lng) {
        var result = getGeoCache(lat, lng);;
    ...
    var getGeoCache = function(lat, lng) {
        var latRounded = roundValue(lat);
        var lngRounded = roundValue(lng);

        var geonames = getLocal("geonames");

        return geonames[latRounded] ? geonames[latRounded][lngRounded] : undefined;
    };

    var setGeoCache = function(lat, lng, value) {
        var latRounded = roundValue(lat);
        var lngRounded = roundValue(lng);
    ...
    var getLocal = function(varname) {
        var result;

        if(localStorage && localStorage[varname]) {
            result = JSON.parse(localStorage[varname]);
        }

        return result;
    };

    var setLocal = function(varname, value) {
    ...
    // save these results in localStorage if its available, round to a certain number to increase cache hits
